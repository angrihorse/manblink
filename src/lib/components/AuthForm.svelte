<script lang="ts">
	...
	import { routeEmailSignin, routeGoogleSignin } from '$lib/client/router';
...
<div class="lg:aspect-square">
	...
	{:else if page.data.user}
		<div class="flex grow flex-col gap-4 text-stone-800">
			...
			<a
				href="/app"
				class="flex h-16 items-center justify-center rounded-lg bg-rose-500 text-lg font-bold text-white select-none hover:bg-rose-600"
			>
				Open the App
			</a>
	...
	{:else}
		<div class="flex grow flex-col text-stone-800">
			<button
				onclick={routeGoogleSignin}
				class="flex h-16 cursor-pointer items-center justify-center gap-3 rounded-lg bg-stone-200 text-lg font-bold select-none hover:bg-stone-300"
			>
				{@html googleIcon}
			...
			<form class="flex flex-col gap-3" onsubmit={async () => await routeEmailSignin(email)}>
				<input
					bind:value={email}
					type="email"
					name="email"
					...
					class="h-16 rounded-lg bg-stone-200 px-4 text-lg font-bold placeholder-stone-700 outline-none focus:ring-4 focus:ring-rose-500"
					required
				/>
				<button
					type="submit"
					class="relative flex h-16 cursor-pointer items-center justify-center rounded-lg bg-rose-500 text-lg font-bold text-white select-none hover:bg-rose-600"
				>
					<Mail class="absolute right-6 size-6" strokeWidth={3} />
